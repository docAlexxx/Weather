...
class DetailsViewModel (
    private val liveData: MutableLiveData<CityLoadStatement> = MutableLiveData(),
        ...
        fun getWeatherFromRemoteServer(lat: Double,lon: Double) {
            ...
            repoImpl.getWeatherFromServer(YANDEX_API_URL + YANDEX_API_URL_END_POINT +"?lat=${lat}&lon=${lon}",callback)
        }
        ...
        private val callback = object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                if(response.isSuccessful){
                    response.body()?.let {
                        val json = it.string()
                        liveData.postValue(CityLoadStatement.Success(Gson().fromJson(json, WeatherDTO::class.java)))
                    }
                }else{
                    // TODO HW
                }
